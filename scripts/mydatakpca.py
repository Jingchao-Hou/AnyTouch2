...
out_path = "log/mydataset_start/kpca_cls_sensor.png"
movement_out_path = "log/mydataset_start/kpca_cls_sensor_movement.png"
...
frame14_movement_out_path = "log/mydataset_start_frame14/kpca_cls_sensor_movement.png"
...
ordered_movement_out_path = "log/mydataset_ordered_15_16/kpca_cls_sensor_movement_right.png"
...
full_kernel_pca = KernelPCA(
    n_components=30, kernel="rbf", gamma=10, fit_inverse_transform=True, alpha=0.1
)
X_2d = full_kernel_pca.fit_transform(X)
...
plt.legend()
plt.title("Kernel PCA of Full Dataset")
plt.tight_layout()
...
plt.title("Kernel PCA of Full Dataset by Sensor and Movement")
plt.tight_layout()
...
frame14_kernel_pca = KernelPCA(
    n_components=3, kernel="rbf", gamma=10, fit_inverse_transform=True, alpha=0.1
)
X_frame14_2d = frame14_kernel_pca.fit_transform(X_frame14)
...
plt.legend(ncol=2, fontsize=8)
plt.title("Kernel PCA of Step-14 CLS Features by Sensor and Movement")
plt.tight_layout()
...
ordered_kernel_pca = KernelPCA(
    n_components=3, kernel="rbf", gamma=10, fit_inverse_transform=True, alpha=0.1
)
X_ordered_2d = ordered_kernel_pca.fit_transform(X_ordered)
...
plt.title("Kernel PCA of Ordered Clips by Sensor and Step")
plt.tight_layout()
