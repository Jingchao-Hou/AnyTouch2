...
def load_class_rows(metrics_path: Path) -> tuple[list[dict[str, float]], float]:
    ...
    metrics = metrics_json["cloth_class_metrics"]
...
def plot_class_accuracy(rows: list[dict[str, float]], acc1: float, output_path: Path) -> None:
    ...
    ax.set_title("cloth class accuracy--training-last-checkpoint")
    ax.set_xlabel("Cloth class")
...
def main() -> None:
    ...
    metrics_path = (
        project_dir
        / "log"
        / "cloth-2026-06-04_16-33-46"
        / "cloth_class_metrics_epoch_49.json"
    )
    output_path = project_dir / "datadis" / "cloth" / "cloth_class_accuracy.png"
